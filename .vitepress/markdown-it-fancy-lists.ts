...
export const markdownItFancyListPlugin = (markdownIt: any, options?: MarkdownItFancyListPluginOptions): void => {
	const isSpace = markdownIt.utils.isSpace;

	// Search `[-+*][\n ]`, returns next pos after marker on success
	// or -1 on fail.
	function parseUnorderedListMarker(state: any, startLine: number): { type: "*" | "-" | "+"; posAfterMarker: number } | null {
	...
	// or -1 on fail.
	function parseOrderedListMarker(state: any, startLine: number): { bulletChar: string; hasOrdinalIndicator: boolean, delimiter: ")" | "."; posAfterMarker: number } | null {
	...
	function markTightParagraphs(state: any, idx: number) {
		let i: number, l;
	...
	function analyseMarker(state: any, startLine: number, endLine: number, previousMarker: Marker | null, options: MarkdownItFancyListPluginOptions): Marker | null {
		...
		if (orderedListMarker !== null) {
            ...
            //console.log("testing charCode", charCode, "bulletChar", bulletChar, "delimiter", delimiter);
			...
			if (isCharCodeDigit(charCode)) {
                ...
                //console.log("parsedRomanNumber", parsedRomanNumber, "isValidRoman", isValidRoman, "bulletChar", bulletChar, "preferRoman", preferRoman);
	...
	type MarkerType = "0" | "a" | "a_upper" | "i" | "i_upper" | "#" | "*" | "-" | "+";
	type Marker = {
		isOrdered: boolean;
		isRoman: boolean;
		isAlpha: boolean;
